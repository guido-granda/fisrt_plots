...
for i in range(0,len(redshifts)):
        ...
        vdisk_t           =np.empty(1)
        vhalo_t           =np.empty(1)
        is_central_t      =np.empty(1)  
        title='$f_{gas}$ of satellite galaxies (Mini-SURF-Velociraptor), '+redshifts[i]
        ...
        # set figure
            ...
            with h5py.File(direc,'r') as hf:
                ...
                is_central      =np.array(data.get("is_central"))#Flag to indicate central galaxy
	        ...
	        vhalo_t           =np.append(vhalo_t,vhalo) 
                is_central_t      =np.append(is_central_t,is_central)
        ...
        mcold_t       =mcold_t[1:]
        is_central_t  =is_central_t[1:]

        mstars_total  =mstars_bulge_t+mstars_disk_t
        mtotal        =mstars_total+mcold_t

        id0=(mtotal>0)
        id1=(is_central_t==1)

        fgas=mcold_t[id0]/mtotal[id0]
        

        fgas_min =np.amin(fgas)
        fgas_max =np.amax(fgas)
        

        print(u'The fgas goes from %3.5f to %3.5f' %(fgas_min,fgas_max))
        # finding range of values
        n_data1=np.shape(fgas)[0]
	...
	################# histogram Vdisk############################3
        hist1, bin_edges1= np.histogram(fgas,bin_edges)
	#############################################################
        #idx2=(hist1[idx] >0) 
	# over the volume
